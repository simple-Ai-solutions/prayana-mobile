...
class ActivityMarketplaceAPI {
  ...
  async searchActivities(filters = {}) {
    // Drop empty values before serialising. `new URLSearchParams({ q: undefined })`
    // produces the literal string "q=undefined", so an unset filter would go out
    // as a search for the word "undefined" rather than as no filter at all.
