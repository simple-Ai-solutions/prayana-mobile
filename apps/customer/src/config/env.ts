/**
 * Centralized, type-safe access to env variables.
 *
 * Why this exists:
...
type Env = {
  ...
  // Razorpay (test/live key id — secret stays server-side)
...
function required(name: string, value: string | undefined): string {
  if (!value) {
    // In dev: throw immediately so the developer notices.
    // In prod: this should never trigger because EAS build inlines env at build time.
    if (__DEV__) {
      throw new Error(
        `[env] Missing required env var: ${name}. Check apps/customer/.env`,
      );
    }
...
// Quick accessor for the common case (avoids `ENV.apiUrl` everywhere)
export const API_URL = ENV.apiUrl;
export const SOCKET_URL = ENV.socketUrl;
export const RAZORPAY_KEY_ID = ENV.razorpayKeyId;
